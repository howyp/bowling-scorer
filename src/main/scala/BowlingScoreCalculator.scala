trait BowlingScoreCalculator {
  def score(frames: List[Frame]): Int = frames match {
    ...
    case Spare(_)          :: remain  => 10 + twoBalls(remain).head + score(remain)
    case Strike            :: remain  => 10 + twoBalls(remain).sum  + score(remain)
    case Nil | Bonus(_, _) :: _       => 0
  }

  def twoBalls(frames: List[Frame]): Stream[Int] = frames match {
    ...
    case Strike          :: remain => 10 #:: twoBalls(remain).head #:: Stream.Empty
